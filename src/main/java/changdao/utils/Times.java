package changdao.utils;


import changdao.utils.exception.ResponseException;
...
public class Times {
    ...
    public static String subStringDate(String value) {
        if (StringUtils.isEmpty(value) || value.length()<10){
            throw new ResponseException("时间格式:"+value+"不正确，请输入正确的时间格式");
        }
        value = value.substring(0, 10);
        Times.parseDate(value);
        return value;
    }
    /**
     * 将日期字符串截取yyyy-MM
     ...
     * @return
     */
    public static String subStringMonth(String value) {
        if (StringUtils.isEmpty(value) || value.length()<10){
            throw new ResponseException("时间格式:"+value+"不正确，请输入正确的时间格式");
        }
        value = value.substring(0, 10);
...
//        if (!checkFormatIsTrue(monthes, TimeType.Month.getTimeTypeNo())) throw new ResponseException("判断时间时出错");
...
//        String[] newMonthes = Arrays.copyOf(monthes, monthes.length * 3);
        ...
        try {
            ...
            throw new ResponseException("请输入正确的时间格式");
        }
        return newMonthes;
    }
    ...
    public static LocalDate parseDate(String dateStr) {
        ...
        if (dateStr.length() == 8) {
            ...
            throw new ResponseException("格式:"+init+"不正确,请输入正确的时间格式");
        }
        return date;
    }
    ...
    public static boolean checkFormatIsTrue(String[] times, String timeType) {
        try {
            ...
            throw new ResponseException("判断时间时出错");
        }
